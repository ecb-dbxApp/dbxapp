...
// namespace Verot\Upload;  #dbXapp use it as system class 
...
class dbxUpload {
     ...
     *
     * Simple wildcards are allowed for MIME types, for example `image all` or `application all`
     ...
     * To only check for forbidden MIME types, and allow everything else, set {@link allowed} to an array containing the all-types wildcard
     *
     ...
     *
     * PNG and WEBP overlays are merged pixel by pixel so transparent areas stay
     * transparent. The destination image is modified by reference and also
     * returned for fluent internal use.
     *
     * Example:
     *
     * ```php
     ...
     * ```
     *
     * Result: `$watermark` is blended into `$dst` at position `20/20` with
     * 60 percent opacity.
     *
     * @access private
     * @param resource|\GdImage $dst_im Destination image, modified in place.
     * @param resource|\GdImage $src_im Source/overlay image.
     * @param int $dst_x X-coordinate in the destination image.
     * @param int $dst_y Y-coordinate in the destination image.
     * @param int $src_x X-coordinate in the source image.
     * @param int $src_y Y-coordinate in the source image.
     * @param int $src_w Width of the source area.
     * @param int $src_h Height of the source area.
     * @param int $pct Overlay opacity from 0 to 100.
     * @return resource|\GdImage Destination image.
     */
                    ...
                    if (!empty($this->image_text)) {
                        ...
                        // get the text bounding box (TrueType fonts)
                            ...
                            if ($rect) {
                                ...
                                $text_offset_x = abs($min_x);
                                $text_offset_y = abs($min_y);
                                $text_width = $max_x - $min_x + (2 * $this->image_text_padding_x);
                                $text_height = $max_y - $min_y + (2 * $this->image_text_padding_y);
                            }
                        }
    ...
    function imagebmp(&$im, $filename = "") {
        ...
        $bi_bpline = $w * 3;
        $bi_stride = ($bi_bpline + 3) & ~3;
        $bi_size_image = $bi_stride * $h;
        $bf_off_bits = 54;
        $bf_size = $bf_off_bits + $bi_size_image;

        $result .= substr('BM', 0, 2);
        $result .=  pack ('VvvV', $bf_size, 0, 0, $bf_off_bits);
        $result .= pack ('VVVvvVVVVVV', 40, $w, $h, 1, 24, 0, $bi_size_image, 0, 0, 0, 0);

        $numpad = $bi_stride - $bi_bpline;
        for ($y = $h - 1; $y >= 0; --$y) {
            for ($x = 0; $x < $w; ++$x) {
