...
foreach (glob($root . '/dbx/modules/dbxContent/include/dbxContentRenderer*Trait.trait.php') ?: array() as $trait_file) {
...
$check(str_contains($cms, 'dbxContentMediaUsageScope::with_language'), 'CMS usage reads and writes must be language scoped.');
$check(str_contains($renderer, 'dbxContentMediaUsageScope::with_language'), 'Frontend rendering must be language scoped.');
$check(
...
$check(str_contains($shop, "'content_lng' => \$this->shop_media_usage_lng()"), 'Shop usage must be assigned to the stable master language.');
$check(str_contains($shop, "\$db->delete(") && str_contains($shop, 'source_needle'), 'Shop synchronization must replace its own snapshot instead of accumulating inactive rows.');
