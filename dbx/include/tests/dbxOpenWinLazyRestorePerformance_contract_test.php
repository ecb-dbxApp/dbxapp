...
$openWin = (string)file_get_contents($root . '/js/lib/openWin.js');
$failures = array();
...
$assert(
    str_contains($openWin, 'const deferContent = !isMobileViewport && restoreMinimized && !hasInlineContent;')
        && str_contains($openWin, 'contentState: deferContent ? "deferred" : "idle"')
        && str_contains($openWin, '} else if (!deferContent) {')
        && str_contains($openWin, 'this.ensureWindowContent(windowData);'),
    'Minimierte Wiederherstellungen laden ihren Inhalt noch beim Seitenstart.'
);
$assert(
    str_contains($openWin, 'ensureAjaxReady(timeout = 5000)')
        && str_contains($openWin, 'this.ajaxReadyPromise')
        && str_contains($openWin, 'dbx.add_js("lib", "ajax.js")')
        && str_contains($openWin, 'return this.ensureAjaxReady().then(ready => {'),
    'openWin wartet nicht zentral und dedupliziert auf seine ajax.js-Abhaengigkeit.'
);
$assert(
    str_contains($openWin, 'isCurrentContentRequest(windowData, requestToken)')
        && str_contains($openWin, 'windowData.contentRequestToken = (windowData.contentRequestToken || 0) + 1;'),
    'Veraltete AJAX-Antworten koennen geschlossene oder neu geladene Fenster noch veraendern.'
);
$assert(
    str_contains($openWin, 'removeCacheBust(url)')
        && str_contains($openWin, 'params.delete("_")'),
    'Wiederholtes Neuladen darf Cache-Buster nicht dauerhaft an die Fenster-URL anhaengen.'
);
$assert(
    str_contains($openWin, 'class="dbx-window-loading" role="status" aria-live="polite"')
        && str_contains($openWin, 'class="dbx-window-error" role="alert"')
        && str_contains($openWin, 'this.escapeHtml(message)'),
