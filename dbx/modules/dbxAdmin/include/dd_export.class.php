...
class dd_export extends \dbxObj {
...
public function export() {
   ...
   dbx()->debug("run export dd_export section=($section)");
   ...
   $run_lines=$this->get_property('run_lines',1000,$section);
   ...
   dbx()->debug("### Export CSV dd=($dd) file=($path_file)");

   $oForm              = dbx()->get_system_obj('dbxForm');
   $oForm->init($section,'form-csv-writer');
   $oForm->_action     = "?dbx_modul=dbxAdmin&dbx_run1=datadic&dbx_run2=export_csv&dd=$dd";
   $oForm->_data       = $data;
   $oForm->_msg_info   = '';
   ...
   if ($submit) {
       ...
       $status = $oImporter->init($dd.'_export_csv');
       ...
       if ($status == 'init') {
           ...
           $oImporter->set_property('run_lines' , $run_lines);
