...
(function (window, document) {
    ...
    function bindUiDefaultControls(root) {
        const panel = root && root.matches && root.matches("[data-admin-ui-defaults='1']")
            ? root
            : (root && root.querySelector ? root.querySelector("[data-admin-ui-defaults='1']") : null);
        if (!panel || panel.__dbxUiDefaultsBound) return;
        panel.__dbxUiDefaultsBound = true;

        const context = dbx.uiDefaultContext && dbx.uiDefaultContext() === "mobile"
            ? "mobile"
            : "desktop";
        const values = dbx.uiCollectDefaultCandidates
            ? dbx.uiCollectDefaultCandidates(context)
            : {};
        const keys = Object.keys(values).sort((a, b) => a.localeCompare(b, "de"));
        const contextLabel = panel.querySelector("[data-ui-default-context]");
        const countLabel = panel.querySelector("[data-ui-default-current-count]");
        const preview = panel.querySelector("[data-ui-default-preview]");
        const save = panel.querySelector("[data-ui-default-save]");

        panel.querySelectorAll("[data-ui-default-context-input]").forEach(input => {
            input.value = context;
        });
        if (contextLabel) contextLabel.textContent = context === "mobile" ? "Mobile" : "Desktop";
        if (countLabel) countLabel.textContent = String(keys.length);
        if (save) save.disabled = keys.length === 0;

        if (preview) {
            preview.replaceChildren();
            if (!keys.length) {
                const empty = document.createElement("div");
                empty.className = "dbx-admin-ui-defaults-empty";
                empty.textContent = "Noch keine freigegebenen persönlichen UI-Werte vorhanden.";
                preview.appendChild(empty);
            } else {
                keys.forEach(storageKey => {
                    const row = document.createElement("div");
                    row.className = "dbx-admin-ui-defaults-row";
                    const code = document.createElement("code");
                    code.textContent = storageKey.replace(/^dbx\.UI\./, "");
                    const value = document.createElement("span");
                    value.textContent = JSON.stringify(values[storageKey]);
        ...
        const saveForm = panel.querySelector("[data-ui-default-form='save']");
        if (saveForm) {
            saveForm.addEventListener("submit", event => {
                const current = dbx.uiCollectDefaultCandidates
                    ? dbx.uiCollectDefaultCandidates(context)
                    : {};
                ...
                const json = saveForm.querySelector("[data-ui-default-json]");
                if (json) json.value = JSON.stringify(current);
            });
        }

        const clearForm = panel.querySelector("[data-ui-default-form='clear']");
        if (clearForm) {
            clearForm.addEventListener("submit", event => {
                if (!window.confirm("Admin-Standard für diesen Darstellungskontext wirklich löschen?")) {
                    event.preventDefault();
                }
            });
        }

        const clearPersonal = panel.querySelector("[data-ui-default-clear-personal]");
        if (clearPersonal) {
            clearPersonal.addEventListener("click", () => {
                if (!window.confirm("Eigene freigegebene UI-Werte entfernen und den Admin-Standard verwenden?")) return;
                if (dbx.uiClearPersonalDefaults) dbx.uiClearPersonalDefaults(context);
                window.location.reload();
            });
        }

        panel.querySelectorAll("[data-ui-default-load]").forEach(button => {
            const buttonContext = button.getAttribute("data-ui-default-load") === "mobile" ? "mobile" : "desktop";
            if (buttonContext !== context) {
                button.disabled = true;
                button.title = "Dieser Standard kann im passenden Darstellungskontext aktiviert werden.";
            }
            button.addEventListener("click", () => {
                const target = button.getAttribute("data-ui-default-load") === "mobile" ? "mobile" : "desktop";
                const source = panel.querySelector(`[data-ui-default-snapshot="${target}"]`);
                if (!source || !dbx.uiApplySnapshot) return;
                try {
                    const snapshot = JSON.parse(source.value || "{}");
                    if (!snapshot || typeof snapshot !== "object" || !Object.keys(snapshot).length) return;
                    dbx.uiApplySnapshot(snapshot, target);
                    window.location.reload();
                } catch (error) {
                    dbx.warn("UI default snapshot invalid", error);
                }
            });
        });
    }
