...
(function (window, document) {
    ...
    * ===================================================== */
    ...
    dbx.uiSet = function (lib, id, key, val) {
        ...
        } catch (e) { dbx.warn("uiSet failed:", k, e); }
    };
    ...
    dbx.add_js = function (type, file, cb) {
        ...
        if (type === "module") {
            ...
            const moduleFile = parts.join("/");
            if (/^[A-Za-z][A-Za-z0-9_]*$/.test(moduleName)
                && /^[A-Za-z0-9][A-Za-z0-9_.-]*\.js$/.test(moduleFile)) {
                url = dbx.config.rootPath + "modules/" + moduleName + "/js/" + moduleFile;
            }
        }
    ...
    function featureScriptUrl(libName, moduleName) {
        const module = String(moduleName || "").trim();
        if (/^[A-Za-z][A-Za-z0-9_]*$/.test(module)) {
            return dbx.config.rootPath + "modules/" + module + "/js/" + libName + ".js";
        }
        return libName === "dbxJstree" ? dbx.config.rootPath + "add_ons/dbxJstree/dbxJstree.js" : dbx.config.libPath + libName + ".js";
    }

    function reloadFeatureScript(libName, moduleName, callback) {
        const url = featureScriptUrl(libName, moduleName);
    ...
    dbx.resolveFeature = function (libName, callback, moduleName) {
        ...
        const exactUrl = featureScriptUrl(libName, moduleName);
        ...
        dbx.loader.js(exactUrl, function (res) {
            ...
            waitForFeatureRegistration(libName, function (registered) {
                ...
                if (libName.includes("-")) {
                    ...
                    const baseUrl = featureScriptUrl(base, moduleName);
                    ...
                    reloadFeatureScript(libName, moduleName, function (reloaded) {
                        if (reloaded) {
   ...
   /* =====================================================
    ...
    dbx.loadFeature = function (name, cb, moduleName) {
        dbx.resolveFeature(name, cb || function () {}, moduleName);
    };
