/*!
 * dbxJstree 1.0.0
 * Complete dbxapp tree library for jQuery 4.x.
 * Based on jsTree 3.3.17 by Ivan Bozhanov, MIT licensed.
 */
...
(function (factory) {
	...
	"use strict";
    var dbx_jquery_major = parseInt(String($ && $.fn && $.fn.jquery || '0').split('.')[0], 10);
    if(dbx_jquery_major !== 4) {
        throw new Error('dbxJstree 1.0.0 requires jQuery 4.x');
    }
/*!
 * dbxJstree 1.0.0 (based on jsTree 3.3.17)
 * http://jstree.com/
 *
...
/*jshint -W083 */
	...
	var dbxJstreeRequest = function (settings) {
		settings = settings || {};
		if(!window.dbx || !window.dbx.ajax || typeof window.dbx.ajax.request !== 'function') {
			var missing = Promise.reject(new Error('dbxJstree requires ajax.js for network data'));
			missing.abort = function () {};
			return missing;
		}
		var dataType = String(settings.dataType || '').toLowerCase();
		return window.dbx.ajax.request({
			url : settings.url || '',
			method : settings.method || settings.type || 'GET',
			mode : dataType === 'json' ? 'json' : (dataType === 'html' || dataType === 'text' ? 'text' : 'auto'),
	...
	$.jstree = {
		 ...
		 */
		plugins : {},
	path : src && src.indexOf('/') !== -1 ? src.replace(/\/[^\/]+$/,'') + '/dbxJstree' : 'dbx/js/lib/dbxJstree',
		idregex : /[\\:&!^|()\[\]<>@*'+~#";.,=\- \/${}%?`]/g,
		root : '#'
	};
		...
		_load_node : function (obj, callback) {
			...
			if(typeof s === 'object') {
				if(s.url) {
					...
					var request = dbxJstreeRequest(s);
					request.then(function (d) {
								...
								this._data.core.last_error = { 'error' : 'ajax', 'plugin' : 'core', 'id' : 'core_04', 'reason' : 'Could not load node', 'data' : JSON.stringify({ 'id' : obj.id, 'response' : d }) };
		...
		this.search = function (str, skip_async, show_only_matches, inside, append, show_only_matches_children) {
			...
			if(!skip_async && a !== false) {
				if($.vakata.is_function(a)) {
					...
					this._data.search.lastRequest = dbxJstreeRequest(a);
					this._data.search.lastRequest.then(function (d) {
							if(d && d.d) { d = d.d; }
							this._load_nodes(!$.vakata.is_array(d) ? [] : $.vakata.array_unique(d), function () {
								this.search(str, true, show_only_matches, inside, append, show_only_matches_children);
							});
 ...
 */
	...
	$.jstree.plugins.state = function (options, parent) {
		this.bind = function () {
			parent.bind.call(this);
			this._data.state = this._data.state || { saveTimer : false };
			var bind = function () {
				this.element.on(this.settings.state.events, function () {
					if(this._data.state.saveTimer) { clearTimeout(this._data.state.saveTimer); }
					this._data.state.saveTimer = setTimeout(function () {
						this._data.state.saveTimer = false;
						this.save_state();
					}.bind(this), 100);
				}.bind(this));
				/**
		...
		this.teardown = function () {
			if(this._data.state && this._data.state.saveTimer) {
				clearTimeout(this._data.state.saveTimer);
				this._data.state.saveTimer = false;
			}
			parent.teardown.call(this);
		};
...
/* dbxapp lib registration and stable lifecycle API. */
(function (window, $) {
	"use strict";
	if(!$ || !$.jstree || !$.fn.jstree) return;

	function optionsFromElement(element, cfg) {
		var options = cfg && typeof cfg.options === 'object' ? cfg.options : null;
		var raw;
		if(!options && element && element.getAttribute) {
			raw = element.getAttribute('data-jstree-options');
	...
	window.dbxJstree = {
		version : '1.0.0',
		upstreamVersion : '3.3.17',
		jqueryCompatibility : '4.x',
		create : function (element, options) { return $(element).jstree(options || {}); },
		instance : function (element) { return $.jstree.reference(element); },
	...
	window.dbx.feature.register('dbxJstree', {
		prio : 'mid',
		scope : 'element',
		js : [
			['js', 'lib', 'ajax.js']
		...
		css : [
			['css', 'root', 'js/lib/dbxJstree/themes/default/style.min.css']
		],
		init : function (element, cfg) {
			return window.dbxJstree.create(element, optionsFromElement(element, cfg || {}));
		},
		destroy : function (element) { window.dbxJstree.destroy(element); }
	});
