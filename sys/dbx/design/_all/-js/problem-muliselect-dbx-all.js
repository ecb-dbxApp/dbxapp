...
function dbxInitModal() {
    $(document).off('click', '.openModal'); // Entfernt vorhandene Event-Listener
    $(document).on('click', '.openModal', function(e) {
        e.preventDefault();

        var target = $(this).data('dbx_target') || '#defaultTarget';
        var href = $(this).data('dbx_get') || $(this).prop('href');

        if (!target || !href) {
            console.error("Fehler: Ziel oder URL fehlt.");
            return false;
        }

        $(target).html('<div class="loading">Wird geladen...</div>'); // Ladeanzeige
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                $(target).html("<p>Fehler beim Laden des Inhalts. Bitte versuchen Sie es später erneut.</p>");
            }
        });
