...
function dbxAjaxAutoSubmit(id,fld = 'no_obs',val = '') {
  ...
  //if (!xTarget) 
  //alert("xx Form=(" + id + ") Target=(" + xTarget + ") action=(" + xGet + ")");
...
function dbxAjaxInitHref() {
  ...
  $('a.dbxAjax').on('click', function(e) {
      ...
      if ($this.hasClass('dbxConfirm')) {
          ...
          $('#dbxConfirmModal .modal-title').text(confirmTitle);
          $('#dbxConfirmModal .modal-body').text(confirmMessage);
...
function autosave_multiselect(id,form) {
  id  =dbx_check_is_id(id);
    ...
    $('input.form-check-input-multi').on('click', function(e) {
       ...
       $.ajax({
            ...
            success : function( response ) {
               ...
               //alert("retval=" + response + ' Len=(' + len + ')  target=(' + target +')');
            },
            error : function( response ) {
...
function dbx_counter_submit() {
    $('.counterSubmit').each(function () {
      const button = $(this);
  
      // Verhindere doppelte Initialisierung
      if (button.data('dbx_initialized')) return;
      button.data('dbx_initialized', true); // Markiere diesen Button als behandelt
  
      const form = button.closest('form'); // Das zugehörige Formular finden
      const originalText = button.text(); // Original-Button-Text speichern
      const countdownSeconds = parseInt(button.data('dbx_counter_sec'), 10); // Countdown-Wert auslesen
      let intervalId = null; // Interval-ID für den Countdown speichern
      let isCounting = false; // Verhindert mehrfaches Starten des Countdowns
  
      // Funktion zum Starten des Countdowns
      const startCountdown = () => {
        if (isCounting) return; // Countdown nur einmal starten
        isCounting = true;
  
        let counter = countdownSeconds; // Lokale Kopie des Counters
        button.text(`${originalText} (${counter})`);
  
        // Countdown starten
        intervalId = setInterval(() => {
          counter--;
          if (counter > 0) {
            button.text(`${originalText} (${counter})`); // Countdown-Anzeige aktualisieren
          } else {
            clearInterval(intervalId); // Countdown stoppen
            button.text('Submitting...');
            isCounting = false; // Countdown-Status zurücksetzen
            form.trigger('submit'); // Formular-Submit auslösen
          }
        }, 1000);
      };
      ...
      button.on('click', function (e) {
        e.preventDefault(); // Standard-Click-Aktion verhindern
        if (intervalId) clearInterval(intervalId); // Eventuelle aktive Intervalle stoppen
        button.text('Submitting...');
        form.trigger('submit'); // Formular-Submit auslösen
      });
  
      // Countdown bei Seitenladebeginn starten (falls gewünscht)
...
function dbx_MultiSelect(elementId, formId) {
    // Referenzen für Select-Element und Form holen
    const $selectElement = $(`#${elementId}`);
    const $formElement = $(`#${formId}`);

    if ($selectElement.length === 0 || $formElement.length === 0) {
        console.error("Element oder Form nicht gefunden.");
        return;
    }

    // Eventlistener für Klick auf Optionen
    $selectElement.on('mousedown', 'option', function (event) {
        event.preventDefault(); // Standard-Mehrfachauswahl deaktivieren
        ...
        // Optische Hervorhebung für Bootstrap
        if (!isSelected) {
            $option.addClass('bg-primary text-white');
        } else {
            $option.removeClass('bg-primary text-white');
        }

        // Bei Optionen mit Klasse "changeSubmit" das Form-Submit auslösen
        if ($option.hasClass('changeSubmit')) {
            $formElement.submit();
        }
    });

    // Initialzustand für optische Hervorhebung setzen
    $selectElement.find('option:selected').addClass('bg-primary text-white');
}

function _dbx_MultiSelect2submit() {
    $(document).on('click', '.dbxMultiSelect2', function () {
        const $li = $(this);
        alert('hallo');
        // Prüfen, ob das Element von "nicht ausgewählt" zu "ausgewählt" wechselt
        if (!$li.hasClass('ms-selected')) {
            // Wechsel zu "ausgewählt" simulieren
            $li.addClass('ms-selected').removeAttr('style');

            // Suchen des zugehörigen Formulars
            const $form = $li.closest('form');

            if ($form.length) {
                $form.submit(); // Formular absenden
            } else {
                console.error("Kein zugehöriges Formular gefunden.");
            }
        }
    });
}
