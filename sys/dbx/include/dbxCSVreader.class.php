...
class dbxCSVreader extends dbxObj {
  ...
  private function make_where($where,$record) {     
   foreach ($record as $name => $value) {
        $xkey='{'.$name.'}';
        $where=str_replace($xkey,$value,$where);
  ...
  public function import() {
    ...
    $all        = $this->get_property('records_all',0);     
    $sep        = $this->get_property('seperator',';'); 
    ...
    //dbx_debug("#IMPORT File=($file) Pos=($filePos) von ($filesize) Lines=($lineCount) ");
    ...
    while( $deadline >= time() AND ( $line=fgets($fp, $bytes ) ) ) {
       ...
       if (!$lineCount) {
          ...
          if ($dd) {
            ...
            if ($pass) {
               if (isset($record['pass'])) $record['pass']=md5($record['pass']);
            }
            
            $where = $this->get_property('where');
            if ($where) {
                $where = $this->make_where($where,$record);
                $ok=$oDB->save($dd,$record,$where,0,1,0,1); //  save($dd,$field_values,$where,$verify_access=1,$verify_fields=1,$verify_values=1,$trace=1)
            } else {
               $ok=$oDB->insert($dd,$record,0,1,0,1);       //insert($dd,$field_values,$verify_access=1,$verify_fields=1,$verify_values=1,$trace=1) {
            }    

            if (!$ok) $errors++;
            if ( $ok) $queryCount++;

            //dbx_debug("IMPORT CSV ($dd) OK=($ok) Where=($where) Querys=($queryCount) errors=($errors) Record=",$record);
