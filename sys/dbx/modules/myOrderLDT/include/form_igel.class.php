...
Class form_igel extends editAnf {
  ...
  public function edit() {
    ...
    if ($rid) {
     ...
     $oForm->init('form-anforderungen-igel');
     ...
     $oForm->_action     = '?dbx_modul=myOrderLDT&dbx_action=order&dbx_work=edit_anf_igel&rid='.$rid;
   ...
   if ($autoSubmit) {
    ...
    $js.= "dbx_MultiSelect('$fld_id','$form_id');\n";  // eigentlich immer auch ohne submit subit ist class bei optionen changeSubmit
    $js.= "dbx_MultiSelect2submit();";
    $oForm->add_js($js);
  }
   $content=$oForm->run();
   return $content;  
