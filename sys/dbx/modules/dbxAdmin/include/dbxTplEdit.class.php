...
Class dbxTplEdit {
   ...
   private function save_tpl($type,$modul,$file,$design,$lng,$tpl) {
      ...
      $file_type=$file.'.'.$type;
      if ($modul == 'dbx') $dir_file=dbx_get_base_dir()."dbx/tpl/$type/$file_type";
      if ($modul != 'dbx') $dir_file=dbx_get_base_dir()."dbx/modules/$modul/tpl/$type/$file_type";
   ...
   public function run($rid=0) {
      ...
      $modul =dbx_get_PostGetVar('modul' ,''   ,'parameter');
      $type  =dbx_get_PostGetVar('type'  ,'htm','parameter');
      $file  =dbx_get_PostGetVar('tpl'   ,''   ,'parameter');
      ...
      $oForm->_data      = $data;
      $oForm->_action    ='?dbx_modul=dbxAdmin&dbx_action=_edittpl';   // &modul='.$modul.'&file='.$file.'&design='.$design.'&lng='.$lng;
      $oForm->_msg_info  = 'TPL: '.$modul.'/'.$file.'.'.$type;
      //$oForm->_editor_fld='tpl';

      $oForm->add_fld('modul' ,'text-label'   ,''       ,'parameter' ,'TPL-Modul');
      $oForm->add_fld('file'  ,'text-label'   ,''       ,'parameter' ,'TPL-File');
      $oForm->add_fld('tpl'   ,'textarea-tpl' ,'rows=22','*'         ,'TPL-Content');
